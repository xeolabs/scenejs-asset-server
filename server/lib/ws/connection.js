...
function Connection(server, req, socket, upgradeHead) {
    ...
    if (this.debug) {
        debug = function () {
            sys.error('\033[90mWS: ' + Array.prototype.join.call(arguments, ", ") + "\033[39m");
        };
    } else {
        debug = function () {
        };
    ...
    if (!checkVersion(this)) {
        ...
        debug(this._id, this.version + " connection");
...
Connection.prototype.write = function(data) {
    ...
    if (this._state == 4) {
        ...
        if (
                writeSocket(socket, "\u0000", "binary") &&
                writeSocket(socket, data, "utf8") &&
                writeSocket(socket, "\uffff", "binary")
                ) {
            return true;
        } else {
            debug(this._id, "\033[31mERROR: write: " + data);
        }
    } else {
...
Parser.prototype.write = function(data) {
    ...
    for (var i = 0, len = data.length; i < len; i++) {
        if (this.order == 0) {
            ...
            debug(this.client._id, "High Order packet handling is not yet implemented.");
            this.order = 0;
        }
    }
};
